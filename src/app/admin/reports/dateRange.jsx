...
const DateRange = () => {
  const startDate = useFilterSliceKey(
    (state) => state.filters[FILTER_MODES.REPORTS]?.startDate
  );
  const endDate = useFilterSliceKey(
    (state) => state.filters[FILTER_MODES.REPORTS]?.endDate
  );
  ...
  const handleApply = () => {
    const range = dateRange[0];
      setFilterValue(FILTER_MODES.REPORTS, "startDate", range.startDate);
    setFilterValue(FILTER_MODES.REPORTS, "endDate", range.endDate);
    handleClose();
  };
